...
@Component({
  ...
  template: `
    <nav class="rounded-xl border border-[#DDE2EA] bg-white p-2.5" aria-label="Primary navigation">
      <div class="grid gap-4">
        @for (section of visibleSections(); track section.id) {
          <section class="grid gap-1">
            <div
              class="flex items-center justify-between px-1.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#98A2B3]"
            >
      ...
      @switch (iconConfig.kind) {
        ...
        @default {
          @switch (iconConfig.name) {
            ...
            @case ('users') {
              <svg lucideUsers class="shrink-0 {{ iconClass }}" aria-hidden="true"></svg>
            }
            @case ('projects') {
              <svg lucideFolderKanban class="shrink-0 {{ iconClass }}" aria-hidden="true"></svg>
            }
            @case ('activity') {
              <svg lucideActivity class="shrink-0 {{ iconClass }}" aria-hidden="true"></svg>
            }
            @case ('settings') {
