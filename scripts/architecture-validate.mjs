...
for (const file of walk(sourceRoot).filter((path) => path.endsWith('.ts'))) {
  ...
  if (imports.some((importPath) => importPath.startsWith('@ngrx/'))) {
    fail(
      file,
      'NgRx is not installed in this reference project; add it deliberately and update the project state standard before importing it',
    );
  }
  ...
  if (rel.startsWith('features/')) {
    ...
    if (rel.includes('/store/')) {
      fail(file, 'feature state should live in state/ services for this reference project');
    }

    if (/^(actions|reducer|selectors|effects)\.ts$/.test(basename(rel))) {
      fail(
        file,
        'action/reducer/selector/effect files do not match this reference project state layout',
      );
    }
